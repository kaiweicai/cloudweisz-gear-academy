...
fn test_start_game() {
    ...
    assert!(!start_game_result.main_failed(), "Program failed: {:?}", start_game_result);
...
fn test_wordle_game_success() {
    ...
    assert!(start_game_result.contains(&Log::builder().payload(Action::StartGame { user: 2.into() })));

    let wordle_result = program.send(2,Action::CheckWord {user:2.into(),word:"house".to_string()});
    assert!(!wordle_result.main_failed(), "Program failed: {:?}", wordle_result);
    assert!(wordle_result.contains(&Log::builder().payload(Event::WordChecked {
        user: 2.into(),
...
fn test_wordle_game_more_times() {
    ...
    let wordle_result = program.send(2,Action::CheckWord {user:2.into(),word:"human".to_string()});
    let result_event = wordle_result.decoded_log::<Event>();
    println!("word result is:{:?}",result_event);
    assert!(!wordle_result.main_failed(), "Program failed: {:?}", wordle_result);
    assert!(wordle_result.contains(&Log::builder().payload(Event::WordChecked {
    ...
    let wordle_result = program.send(2,Action::CheckWord {user:2.into(),word:"heuan".to_string()});
    let result_event = wordle_result.decoded_log::<Event>();
    println!("word result is:{:?}",result_event);
    assert!(!wordle_result.main_failed(), "Program failed: {:?}", wordle_result);
    assert!(wordle_result.contains(&Log::builder().payload(Event::WordChecked {
